...
static sai_status_t
_ctc_sai_neighbor_add_nexthop(uint8 lchip, sai_object_id_t rif_id, ctc_sai_neighbor_t* p_neighbor_info)
{
    sai_status_t           status = SAI_STATUS_SUCCESS;
    uint8 rif_type = 0;
...
_ctc_sai_neighbor_set_attr(sai_object_key_t* key, const sai_attribute_t* attr)
{
    ...
    if (SAI_NEIGHBOR_ENTRY_ATTR_DST_MAC_ADDRESS == attr->id)
    {
        ...
        CTC_SAI_ERROR_GOTO(_ctc_sai_neighbor_add_nexthop(lchip, neighbor_entry->rif_id, p_neighbor_info), status, error);
...
_ctc_sai_neighbor_hash_traverse_fun(void* bucket_data, void* user_data)
{
    ...
    _ctc_sai_neighbor_add_nexthop(lchip, rif_id, p_neighbor_info);/*lookup fdb again*/
...
ctc_sai_neighbor_alloc_ipuc_nexthop(uint8 lchip, sai_object_id_t rif_id, sai_ip_address_t* ip_address, uint32* nh_id)
{
    ...
    CTC_SAI_ERROR_GOTO(_ctc_sai_neighbor_add_nexthop(lchip, rif_id, p_neighbor_info), status, error1);
    *nh_id = p_neighbor_info->nh_id;
    p_neighbor_info->ref_cnt++;
    goto out;
error1:
    _ctc_sai_neighbor_remove_db(lchip, rif_id, ip_address, 0);
out:
    return status;
}
sai_status_t
                                                          ...
                                                          uint32_t attr_count, const sai_attribute_t *attr_list)
{
    ...
    CTC_SAI_ERROR_GOTO(_ctc_sai_neighbor_add_nexthop(lchip, neighbor_entry->rif_id, p_neighbor_info), status, error1);
