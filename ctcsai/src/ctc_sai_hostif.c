...
_ctc_sai_hostif_port_get_mac(uint8 lchip, uint8* mac, uint32 port_idx)
{
    ...
    uint32 cnt = 0;
    uint16 linkagg_cnt  = 0;
    uint8 tid = 0;
    uint32 gport = 0;

    CTC_SAI_CTC_ERROR_RETURN(ctcs_l3if_get_router_mac(lchip, mac));
    if (CTC_IS_LINKAGG_PORT(port_idx))
    {
        tid = CTC_GPORT_LINKAGG_ID(port_idx);
        CTC_SAI_ERROR_RETURN(ctcs_linkagg_get_1st_local_port(lchip, tid, &gport, &linkagg_cnt));
        cnt = CTC_MAP_GPORT_TO_LPORT(gport);
    }
    else
    {
        cnt = CTC_MAP_GPORT_TO_LPORT(port_idx);
    }

    _ctc_sai_hostif_chsm_mac_addr_add(mac, cnt);
...
sai_status_t
_ctc_sai_hostif_set_mac(uint8 lchip, uint32 gport, const char *ifname, uint32 enable)
{
    mac_addr_t mac;
    ...
    {
        CTC_SAI_ERROR_RETURN(_ctc_sai_hostif_port_get_mac(lchip, mac, gport));
    }
...
ctc_sai_hostif_l3if_en(uint8 lchip, uint32 gport, uint32 enable)
{
    ...
    CTC_SAI_ERROR_RETURN(_ctc_sai_hostif_set_mac(lchip, gport, p_hostif_port->ifname, enable));
        ...
        _In_ const sai_attribute_t *attr_list)
{
    ...
    if (SAI_HOSTIF_TYPE_NETDEV == attr_type_val->s32)
    {
        ...
        if (SAI_OBJECT_TYPE_VLAN != ctc_oid.type)
        {
            ...
            CTC_SAI_ERROR_GOTO(_ctc_sai_hostif_set_mac(lchip, ctc_oid.value, p_hostif_port->ifname, enable), status, roll_back_3);
        }
        ...
        _In_ const sai_attribute_t *attr_list)
{
    ...
    if (SAI_HOSTIF_TX_TYPE_PIPELINE_BYPASS == attr_val->s32)
    {
    ...
    else if (SAI_HOSTIF_TX_TYPE_PIPELINE_LOOKUP == attr_val->s32)
    {
