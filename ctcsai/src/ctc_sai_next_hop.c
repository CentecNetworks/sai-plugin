...
_ctc_sai_next_hop_add_mpls(uint8 lchip, uint32 nh_id, sai_u32_list_t label, ctc_sai_next_hop_t* p_next_hop_info)
{
    ...
    if(0 == label.count)/*php*/
    {
        ...
        CTC_SAI_ERROR_RETURN(ctcs_nh_add_mpls(lchip, nh_id, &nh_mpls_param));
    }
    else
    {
        CTC_SAI_ERROR_RETURN(ctc_sai_db_alloc_id(lchip, CTC_SAI_DB_ID_TYPE_TUNNEL_ID, &tunnel_id));
        ...
        CTC_SAI_CTC_ERROR_GOTO(ctcs_nh_add_mpls_tunnel_label(lchip, tunnel_id, &nh_tunnel_param), status, error2);

        nh_mpls_param.nh_prop = CTC_MPLS_NH_PUSH_TYPE;
        nh_mpls_param.nh_para.nh_param_push.nh_com.opcode = CTC_MPLS_NH_PUSH_OP_ROUTE;
        nh_mpls_param.nh_para.nh_param_push.tunnel_id = tunnel_id;
        if (3 == label.count)
        {
        ...
        CTC_SAI_CTC_ERROR_GOTO(ctcs_nh_add_mpls(lchip, nh_id, &nh_mpls_param), status, error3);
        p_next_hop_info->label.count = label.count;
        sal_memcpy(p_next_hop_info->label.list, label.list, sizeof(uint32_t)*label.count);
        p_next_hop_info->ctc_mpls_tunnel_id = tunnel_id;
    }
...
error3:
    ...
    ctcs_nh_remove_mpls_tunnel_label(lchip, tunnel_id);
error2:
...
error1:
    ...
    ctc_sai_db_free_id(lchip, CTC_SAI_DB_ID_TYPE_TUNNEL_ID, tunnel_id);
    return status;
}
...
_ctc_sai_next_hop_create_attr_check(uint8 lchip, uint32_t attr_count, const sai_attribute_t *attr_list)
{
    ...
    {
        CTC_SAI_ERROR_GOTO(ctc_sai_find_attrib_in_list(attr_count, attr_list, SAI_NEXT_HOP_ATTR_TUNNEL_ID, &attr_value, &index), status, error);
        p_oid_property = ctc_sai_db_get_object_property(lchip, attr_value->oid);
        if (NULL == p_oid_property)
        {
            return SAI_STATUS_ITEM_NOT_FOUND;
...
static sai_status_t
_ctc_sai_next_hop_set_attr_check(uint32_t attr_count, const sai_attribute_t *attr_list)
{
    sai_status_t           status = SAI_STATUS_SUCCESS;
    const sai_attribute_value_t *attr_value = NULL;
    uint32_t index = 0;
    ...
    status = (ctc_sai_find_attrib_in_list(attr_count, attr_list, SAI_NEXT_HOP_TYPE_SEGMENTROUTE_SIDLIST, &attr_value, &index));
    if (SAI_STATUS_SUCCESS == status)
    {
        return SAI_STATUS_ATTR_NOT_SUPPORTED_0 + index;
    }
    status = (ctc_sai_find_attrib_in_list(attr_count, attr_list, SAI_NEXT_HOP_TYPE_SEGMENTROUTE_ENDPOINT, &attr_value, &index));
    if (SAI_STATUS_SUCCESS == status)
    {
        return SAI_STATUS_ATTR_NOT_SUPPORTED_0 + index;
    }
...
static sai_status_t
_ctc_sai_next_hop_cmp_tunnel_info(ctc_sai_oid_property_t* bucket_data, ctc_sai_next_hop_t* user_data)
{
    sai_status_t           status = SAI_STATUS_SUCCESS;
...
ctc_sai_next_hop_get_tunnel_nh(sai_object_id_t tunnel_id, const sai_ip_address_t* ip_addr, uint32* tunnel_nh_id)
{
    ...
    ctc_sai_db_traverse_object_property(lchip, SAI_OBJECT_TYPE_NEXT_HOP,
                                                         (hash_traversal_fn)_ctc_sai_next_hop_cmp_tunnel_info, (void*)(&ctc_sai_nh));
...
_ctc_sai_next_hop_wb_reload_cb1(uint8 lchip)
{
    ...
    entry_cnt++;
    sal_memcpy(&next_hop_wb, (uint8*)(wb_query.buffer) + offset,  sizeof(sai_qos_map_t));
                                                       ...
                                                       uint32_t attr_count, const sai_attribute_t *attr_list)
{
    ...
    ctc_sai_find_attrib_in_list(attr_count, attr_list, SAI_NEXT_HOP_ATTR_ROUTER_INTERFACE_ID, &attr_value, &index);
    if (!CTC_SAI_ERROR(status))
    {
        p_next_hop_info->rif_id = attr_value->oid;
    }

    ctc_sai_find_attrib_in_list(attr_count, attr_list, SAI_NEXT_HOP_ATTR_TYPE, &attr_value, &index);
    ...
    else if(SAI_NEXT_HOP_TYPE_MPLS == next_hop_type)
    {
...
ctc_sai_next_hop_set_nh_attr(sai_object_id_t next_hop_id, const sai_attribute_t *attr)
{
    ...
    CTC_SAI_ERROR_RETURN(ctc_sai_oid_get_lchip(next_hop_id, &lchip));
    CTC_SAI_ERROR_RETURN(_ctc_sai_next_hop_set_attr_check(1, attr));
    CTC_SAI_DB_LOCK(lchip);
    CTC_SAI_LOG_ENTER(SAI_API_NEXT_HOP);
