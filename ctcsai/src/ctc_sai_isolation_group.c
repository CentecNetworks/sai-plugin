...
ctc_sai_isolation_group_get_group_property(sai_object_key_t* key, sai_attribute_t* attr, uint32 attr_idx)
{
    ...
    switch(attr->id)
    {
        case SAI_ISOLATION_GROUP_ATTR_TYPE:
            attr->value.u32 = p_ist_grp->ist_type;
            break;
        ...
        _In_ const sai_attribute_t *attr_list)
{
    ...
    CTC_SAI_DB_LOCK(lchip);
    CTC_SAI_ERROR_GOTO(ctc_sai_db_alloc_id(lchip, CTC_SAI_DB_ID_TYPE_COMMON, &p_ist_grp_mem->ist_grp_mem_id), status, roll_back_1);
    *isolation_group_member_id = ctc_sai_create_object_id(SAI_OBJECT_TYPE_ISOLATION_GROUP_MEMBER, lchip, 0, 0, p_ist_grp_mem->ist_grp_mem_id);
    CTC_SAI_ERROR_GOTO(ctc_sai_db_add_object_property(lchip, *isolation_group_member_id, p_ist_grp_mem), status, roll_back_2);
    ...
    CTC_SAI_CTC_ERROR_GOTO(ctcs_port_set_restriction(lchip, ctc_object_id.value, &port_restriction), status, roll_back_4);
...
roll_back_2:
    ctc_sai_db_free_id(lchip, CTC_SAI_DB_ID_TYPE_COMMON, p_ist_grp_mem->ist_grp_mem_id);
