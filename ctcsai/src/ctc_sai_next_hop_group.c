...
_ctc_sai_next_hop_group_get_members_from_db(ctc_sai_oid_property_t* bucket_data, sai_object_list_t* user_data)
{
    ...
    {
        user_data->list[user_data->count++] = ctc_sai_create_object_id(SAI_OBJECT_TYPE_NEXT_HOP_GROUP_MEMBER,
                                                      ctc_object_id.lchip, ctc_object_id.sub_type, 0, ctc_object_id.value);
    }
    return status;
}
static sai_status_t
_ctc_sai_next_hop_group_set_grp_attr(sai_object_key_t* key, const sai_attribute_t* attr)
{
    ...
    if (SAI_NEXT_HOP_GROUP_ATTR_SET_SWITCHOVER == attr->id)
    {
        ...
        CTC_SAI_CTC_ERROR_RETURN(ctcs_aps_set_aps_bridge(lchip, ctc_object_id.value2, attr->value.booldata));
    }
    else
    {
        return SAI_STATUS_NOT_SUPPORTED;
         ...
         sai_status_t *object_statuses)
{
    ...
    for (i = 0; i < object_count; i++)
    {
        CTC_SAI_ERROR_RETURN(ctc_sai_oid_get_lchip(object_id[i], &lchip));
        CTC_SAI_DB_LOCK(lchip);
