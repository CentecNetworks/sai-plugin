...
sai_status_t ctc_sai_mcast_get_member_output_id(sai_object_key_t *key, sai_attribute_t *attr, uint32 attr_idx)
{
    ...
    ctc_sai_get_ctc_object_id(attr->id, key->key.object_id, &ctc_oid);
        ...
        ctc_sai_mcast_ipmc_bind_type_t *bind_type)
{
    ...
    if (p_entry_data)
    {
        ...
        CTC_SAI_ERROR_RETURN(ctc_sai_mcast_get_group_member_list(&key, &attr, 0));
    ...
    else
    {
                ...
                && (bind_node->bind_type == CTC_SAI_MCAST_IPMC_BIND_L2MC)))
        {
            CTC_SLIST_LOOP(p_group_data->output_id_head, node)
            {
                ...
                if ((sai_l3if_type == SAI_ROUTER_INTERFACE_TYPE_VLAN)
                    ||(sai_l3if_type == SAI_ROUTER_INTERFACE_TYPE_SUB_PORT))
                {
                    ctc_sai_router_interface_get_vlan_ptr(po->output_id, &vlan_ptr);
                    if (p_travs_data->opt_type == CTC_SAI_DELETE)
                    ...
                    {/*try bind all vlan interface except binded vlan*/
                        if (vlan_ptr == fid)
                        {
                            continue;
                        }
        ...
        _In_ const sai_attribute_t *attr_list)
{
    ...
    CTC_SAI_ERROR_RETURN(ctc_sai_db_alloc_id(lchip, CTC_SAI_DB_ID_TYPE_SAI_MCAST_GROUP, &grp_id));
    ...
    {
        CTC_SAI_LOG_ERROR(SAI_API_L2MC_GROUP, "l2mc group property exist, group oid: 0x%llx\n", grp_oid);
    ...
    *l2mc_group_id = grp_oid;
    ...
    CTC_SAI_LOG_INFO(SAI_API_L2MC_GROUP, "create l2mc sai group id: %d, oid: 0x%llx\n", grp_id, *l2mc_group_id);
        ...
        _In_ sai_object_id_t l2mc_group_id)
{
    ...
    CTC_SAI_LOG_INFO(SAI_API_L2MC_GROUP, "remove l2mc sai group id: %d, oid: 0x%llx\n", grp_id, l2mc_group_id);
        ...
        _In_ const sai_attribute_t *attr)
{
    ...
    CTC_SAI_ERROR_GOTO(ctc_sai_set_attribute(&key, NULL,
                                             SAI_OBJECT_TYPE_L2MC_GROUP,  l2mc_group_attr_fn_entries, attr), status, out);
        ...
        _In_ const sai_attribute_t *attr_list)
{
    ...
    CTC_SAI_ERROR_RETURN(ctc_sai_find_attrib_in_list(attr_count, attr_list, SAI_L2MC_GROUP_MEMBER_ATTR_L2MC_GROUP_ID, &attr_grp, &attr_idx));
    CTC_SAI_ERROR_RETURN(ctc_sai_find_attrib_in_list(attr_count, attr_list, SAI_L2MC_GROUP_MEMBER_ATTR_L2MC_OUTPUT_ID, &attr_id, &attr_idx));

    p_bridge_port = ctc_sai_db_get_object_property(lchip, attr_id->oid);
    ...
    *l2mc_group_member_id = member_oid;
    CTC_SAI_LOG_INFO(SAI_API_L2MC_GROUP, "create l2mc group member, sai group id: %d, group oid: 0x%llx, output oid: 0x%llx, port: %d, oid: 0x%llx\n",
            grp_id, attr_grp->oid, attr_id->oid, p_bridge_port->gport, *l2mc_group_member_id);
        ...
        _In_ sai_object_id_t l2mc_group_member_id)
{
    ...
    CTC_SAI_LOG_INFO(SAI_API_L2MC_GROUP, "remove l2mc group member, sai group id: %d, port: %d, oid: 0x%llx\n",
...
static sai_status_t ctc_sai_mcast_set_l2mc_member_endpoint_ip(sai_object_key_t *key, const  sai_attribute_t *attr)
{
...
static  ctc_sai_attr_fn_entry_t l2mc_member_attr_fn_entries[] = {
    ...
    { SAI_L2MC_GROUP_MEMBER_ATTR_L2MC_ENDPOINT_IP,
      NULL,
      ctc_sai_mcast_set_l2mc_member_endpoint_ip},
    { CTC_SAI_FUNC_ATTR_END_ID,
      NULL,
      NULL}
};
        ...
        _In_ const sai_attribute_t *attr)
{
    ...
    CTC_SAI_ERROR_GOTO(ctc_sai_set_attribute(&key, NULL,
                                             SAI_OBJECT_TYPE_L2MC_GROUP_MEMBER,  l2mc_member_attr_fn_entries, attr), status, out);
        ...
        _In_ const sai_attribute_t *attr_list)
{
    ...
    if (status == SAI_STATUS_SUCCESS)
    {
        ...
        CTC_SAI_ERROR_RETURN(_ctc_sai_mcast_check_l2mc_member_list(l2mc_entry->bv_id, p_group_data->output_id_head));
    }
    ...
    if (p_group_data)
    {
        ...
        CTC_SLIST_LOOP(p_group_data->output_id_head, node)
        {
            ...
            CTC_SAI_ERROR_RETURN(ctcs_nh_update_mcast(lchip, nh_id, &nh_mcast_group));
        }
        ...
        _In_ const sai_attribute_t *attr_list)
{
    ...
    if (p_entry_data)
    {
        CTC_SAI_LOG_ERROR(SAI_API_MCAST_FDB,"mcast fdb property db is exist\n");
        return SAI_STATUS_ITEM_ALREADY_EXISTS;
    }
    ...
    if (status == SAI_STATUS_SUCCESS)
    {
        ...
        CTC_SAI_ERROR_RETURN(_ctc_sai_mcast_check_l2mc_member_list(mcast_fdb_entry->bv_id, p_group_data->output_id_head));
    }
    ...
    CTC_SAI_LOG_INFO(SAI_API_MCAST_FDB, "create mcast fdb entry, ctc group id: %d, oid: 0x%llx, action: %d, mac: %.2x%.2x.%.2x%.2x.%.2x%.2x, fid: %d\n",
                grp_param.l2mc_grp_id, (attr_grp ? attr_grp->oid : 0), action, grp_param.mac[0], grp_param.mac[1], grp_param.mac[2], grp_param.mac[3], grp_param.mac[4],
...
static sai_status_t ctc_sai_mcast_set_mcast_fdb_action(sai_object_key_t *key, const  sai_attribute_t *attr)
{
    sai_mcast_fdb_entry_t *mcast_fdb_entry = NULL;
    ctc_l2_mcast_addr_t grp_param;
    ctc_sai_mcast_entry_property_t *p_entry_data = NULL;
...
static  ctc_sai_attr_fn_entry_t mcast_fdb_attr_fn_entries[] = {
      ...
      ctc_sai_mcast_get_mcast_fdb_entry_info,
      ctc_sai_mcast_set_mcast_fdb_action},
        ...
        _In_ const sai_attribute_t *attr_list)
{
    ...
    {
        value = (vlan_ptr << 20) | (value & 0xFFFFF);
    }
        ...
        _In_ const sai_attribute_t *attr)
{
    ...
    CTC_SAI_ERROR_GOTO(ctc_sai_set_attribute(&key, NULL,
                                             SAI_OBJECT_TYPE_IPMC_GROUP_MEMBER,  ipmc_group_attr_fn_entries, attr), status, out);
        ...
        _In_ sai_object_id_t rpf_group_member_id)
{
    ...
    rpf_group_id = ctc_sai_create_object_id(SAI_OBJECT_TYPE_RPF_GROUP, lchip, 0, 0, rpf_group_idx);
    ...
    CTC_SAI_LOG_INFO(SAI_API_RPF_GROUP, "create rpf group member, group id: %d, group oid: 0x%llx, intf id: %d, member oid: 0x%llx\n",
...
sai_status_t ctc_sai_mcast_get_rpf_member_info(sai_object_key_t *key, sai_attribute_t *attr, uint32 attr_idx)
{
    ...
    switch (attr->id)
    {
        case SAI_RPF_GROUP_MEMBER_ATTR_RPF_GROUP_ID:
            attr->value.oid = ctc_sai_create_object_id(SAI_OBJECT_TYPE_RPF_GROUP, lchip, 0, 0, ctc_oid.value2);
            break;
...
ctc_sai_mcast_db_init(uint8 lchip)
{
    ...
    wb_info.data_len = sizeof(ctc_sai_entry_property_t);
    wb_info.wb_sync_cb = NULL;
