...
static sai_status_t
_ctc_sai_route_mapping_ipuc_action(ctc_ipuc_param_t* ipuc_info, sai_packet_action_t action, sai_object_id_t nh_obj_id, uint16 cid)
{
    uint32 ctc_nh_id = 0;
                                             ...
                                             uint32_t attr_count, const sai_attribute_t *attr_list)
{
    ...
    CTC_SAI_ERROR_GOTO(_ctc_sai_route_mapping_ipuc_key(route_entry, &ipuc_info), status, out);
    CTC_SAI_ERROR_GOTO(_ctc_sai_route_mapping_ipuc_action(&ipuc_info, action, nh_obj_id, cid), status, out);
    CTC_SAI_CTC_ERROR_GOTO(ctcs_ipuc_add(lchip, &ipuc_info), status, out);
    CTC_SAI_ERROR_GOTO(_ctc_sai_route_build_db(lchip, route_entry, &p_route_info), status, error1);
...
_ctc_sai_route_remove_route(const sai_route_entry_t *route_entry)
{
    ...
    CTC_SAI_ERROR_RETURN(_ctc_sai_route_mapping_ipuc_action(&ipuc_info, p_route_info->action, p_route_info->nh_obj_id, p_route_info->cid));
...
_ctc_sai_route_set_attr(sai_object_key_t* key, const sai_attribute_t* attr)
{
    ...
    CTC_SAI_ERROR_RETURN(_ctc_sai_route_mapping_ipuc_action(&ipuc_info, action, nh_obj_id, cid));
...
_ctc_sai_next_route_dump_print_cb(ctc_sai_entry_property_t* bucket_data, ctc_sai_db_traverse_param_t *p_cb_data)
{
    ...
    sal_memcpy(&(ip_addr.addr), &(route_entry.destination.addr), sizeof(sai_ip_address_t));
