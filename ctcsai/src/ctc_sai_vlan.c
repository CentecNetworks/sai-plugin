...
ctc_sai_vlan_set_info(sai_object_key_t *key, const sai_attribute_t* attr)
{
    ...
    case  SAI_VLAN_ATTR_STP_INSTANCE:
        CTC_SAI_ERROR_RETURN(ctc_sai_stp_set_instance(lchip, vlan_ptr, attr->value.oid, 1));
    ...
    case  SAI_VLAN_ATTR_LEARN_DISABLE:
        CTC_SAI_CTC_ERROR_RETURN(ctcs_vlan_set_learning_en(lchip, vlan_ptr, attr->value.booldata));
        break;
...
ctc_sai_vlan_get_info(sai_object_key_t *key, sai_attribute_t* attr, uint32 attr_idx)
{
    ...
    case  SAI_VLAN_ATTR_MEMBER_LIST:
        ...
        CTC_SAI_CTC_ERROR_RETURN(ctcs_vlan_get_ports(ctc_object_id.lchip, vlan_ptr, ctc_object_id.lchip, port_bitmap));
        ...
        for (bit_cnt = 0; bit_cnt < capability[CTC_GLOBAL_CAPABILITY_MAX_PORT_NUM]; bit_cnt++)
        {
    ...
    case  SAI_VLAN_ATTR_STP_INSTANCE:
        ...
        attr->value.oid = ctc_sai_create_object_id(SAI_OBJECT_TYPE_STP, ctc_object_id.lchip, 0, 0, stp_id);
        break;
    case  SAI_VLAN_ATTR_LEARN_DISABLE:
...
ctc_sai_vlan_get_vlan_property(sai_object_key_t *key, sai_attribute_t* attr, uint32 attr_idx)
{
    ...
    CTC_SAI_CTC_ERROR_RETURN(ctcs_vlan_get_property(lchip, vlan_ptr, vlan_prop, &attr->value.u32));
...
ctc_sai_vlan_set_vlan_property(sai_object_key_t *key,  const sai_attribute_t* attr)
{
    ...
    CTC_SAI_CTC_ERROR_RETURN(ctcs_vlan_set_property(lchip, vlan_ptr, vlan_prop, value));
...
sai_status_t ctc_sai_vlan_get_vlan_member(sai_object_key_t *key, sai_attribute_t* attr, uint32 attr_idx)
{
    ...
    switch (attr->id)
    {
        case SAI_VLAN_MEMBER_ATTR_VLAN_ID:
            attr->value.oid = ctc_sai_create_object_id(SAI_OBJECT_TYPE_VLAN, lchip, 0, 0, vlanptr);
            break;
...
static sai_status_t ctc_sai_vlan_get_vlan_member_tag(sai_object_key_t *key,  sai_attribute_t* attr, uint32 attr_idx)
{
    ...
    CTC_SAI_CTC_ERROR_RETURN(ctcs_vlan_get_ports(lchip, vlan_ptr, lchip, port_bitmap));
   ...
   /*TODO:SAI_VLAN_TAGGING_MODE_PRIORITY_TAGGED*/
    ...
    CTC_SAI_CTC_ERROR_RETURN(ctcs_vlan_get_tagged_ports(lchip, vlan_ptr, lchip, port_bitmap_tagged));
...
static sai_status_t ctc_sai_vlan_set_vlan_member_tag(sai_object_key_t *key,  const sai_attribute_t* attr)
{
    ...
    if (SAI_VLAN_TAGGING_MODE_PRIORITY_TAGGED == attr->value.s32 )
    {
        /*TODO:SCL PRIORITY_TAGGED*/
    }
    return SAI_STATUS_SUCCESS;
}
                                                     ...
                                                     const sai_attribute_t *attr_list)
{
    ...
    *vlan_member_id = ctc_sai_create_object_id(SAI_OBJECT_TYPE_VLAN_MEMBER, lchip, 0, vlanptr, gport);
    ...
    error1:
    (void)ctcs_vlan_remove_port( lchip, vlanptr, gport);
out:
    return status;
}
...
static sai_status_t _ctc_sai_vlan_remove_vlan_member( sai_object_id_t vlan_member_id)
{
    ...
    vlan_oid = ctc_sai_create_object_id(SAI_OBJECT_TYPE_VLAN, lchip, 0, 0, vlan_id);
...
ctc_sai_vlan_get_vlan_id_from_vlan_ptr(uint8 lchip, uint16 vlan_ptr, uint16 *vlan_id)
{
    ...
    oid = ctc_sai_create_object_id(SAI_OBJECT_TYPE_VLAN, lchip, 0, 0, vlan_ptr);
...
error1:
    ctcs_vlan_destroy_vlan( lchip, vlan.user_vlanptr);
error0:
...
static sai_status_t ctc_sai_vlan_remove_vlan( sai_object_id_t sai_vlan_id)
{
    ...
    stp_oid = ctc_sai_create_object_id(SAI_OBJECT_TYPE_STP, ctc_object_id.lchip, 0, 0, p_dbvlan->stp_id);
    CTC_SAI_ERROR_GOTO(ctc_sai_stp_set_instance(ctc_object_id.lchip, p_dbvlan->user_vlanptr, stp_oid, 0), status, out);
    /*not vlanptr*/
    CTC_SAI_CTC_ERROR_GOTO(ctcs_vlan_destroy_vlan( ctc_object_id.lchip, p_dbvlan->vlan_id), status, out);

    /*flush fdb*/
    switch_id = ctc_sai_create_object_id(SAI_OBJECT_TYPE_SWITCH, ctc_object_id.lchip, 0, 0, ctc_object_id.lchip);
...
out:
    ...
    if (SAI_STATUS_SUCCESS != status)
    {
        CTC_SAI_LOG_ERROR(SAI_API_VLAN, "Failed to get vlan attr:%d, attr_id:%d\n", status, attr_list[loop].id);
    }
    return status;
}
                                                  ...
                                                  const sai_vlan_stat_t *counter_ids)
{
    ...
    for (index = 0; index < number_of_counters; index++)
    {
        if (SAI_VLAN_STAT_IN_PACKETS == counter_ids[index] || SAI_VLAN_STAT_IN_OCTETS == counter_ids[index] )
        {
            if (0 != p_dbvlan->stats_id_in)
            {
                CTC_SAI_ERROR_GOTO(ctcs_stats_clear_stats(lchip, p_dbvlan->stats_id_in), status, out);
            }
        }
        ...
        if (SAI_VLAN_STAT_OUT_PACKETS == counter_ids[index] || SAI_VLAN_STAT_OUT_OCTETS == counter_ids[index] )
        {
            if (0 != p_dbvlan->stats_id_eg)
            {
                CTC_SAI_ERROR_GOTO(ctcs_stats_clear_stats(lchip, p_dbvlan->stats_id_eg), status, out);
            }
        }
    }
