...
typedef struct ctc_sai_lag_info_s
{
    ...
    bool drop_tagged;
    bool drop_untagged;
    ctc_sai_lag_member_change_notification_fn cb[SAI_API_MAX];
}ctc_sai_lag_info_t;
...
{
    const sai_attribute_value_t *lag_id = NULL, *port_id = NULL, *attr = NULL;
    uint32    lag_id_index, port_index, index;
    sai_status_t status;
    sai_status_t status_in;
    sai_status_t status_out;
    uint8 lchip;
...
#if 1
    /* get egress mode */
    status_out = ctc_sai_find_attrib_in_list(attr_count, attr_list, SAI_LAG_MEMBER_ATTR_EGRESS_DISABLE, &attr, &index);

    /* get ingress mode */
    status_in = ctc_sai_find_attrib_in_list(attr_count, attr_list, SAI_LAG_MEMBER_ATTR_INGRESS_DISABLE,
                                            &attr, &index);
#endif
    ...
    else if (SAI_STATUS_SUCCESS == status_out)
    {
        CTC_BMP_SET(db_lag->Egress_disable_ports_bits, CTC_MAP_GPORT_TO_LPORT(ctc_port_oid.value));
        /*0x1f | lag_id*/
        //lag_port = (0x1f << 8) | (ctc_lag_oid.value & 0xff);
    ...
    *lag_member_id = ctc_sai_create_object_id(SAI_OBJECT_TYPE_LAG_MEMBER, lchip, 0, (uint16)ctc_lag_oid.value, ctc_port_oid.value);
    ...
    {
        CTC_SAI_CTC_ERROR_GOTO(ctcs_port_set_phy_if_en(lchip, ctc_port_oid.value, 1), status, error3);
    }
...
ctc_sai_lag_get_info(sai_object_key_t *key, sai_attribute_t* attr, uint32 attr_idx)
{
    ...
    case SAI_LAG_ATTR_PORT_LIST:
        ...
        for (bit_cnt = 0; bit_cnt < sizeof(p_db_lag->member_ports_bits)*8; bit_cnt++)
        {
            ...
            {
                lag_members[port_num] =  ctc_sai_create_object_id(SAI_OBJECT_TYPE_LAG_MEMBER, ctc_object_id.lchip, 0, 0, bit_cnt);
                port_num++;
            }
        }
...
ctc_sai_lag_set_info(sai_object_key_t *key, const sai_attribute_t* attr)
{
    ...
    case  SAI_LAG_ATTR_PORT_VLAN_ID:
        for (bit_cnt = 0; bit_cnt < sizeof(p_db_lag->member_ports_bits)*8; bit_cnt++)
        {
            if (CTC_BMP_ISSET(p_db_lag->member_ports_bits, bit_cnt))
            {
                CTC_SAI_CTC_ERROR_RETURN(ctcs_port_set_default_vlan(lchip, CTC_MAP_LPORT_TO_GPORT(lchip, bit_cnt), attr->value.u16));
            }
        }
        break;
    ...
    case SAI_LAG_ATTR_DEFAULT_VLAN_PRIORITY:
        for (bit_cnt = 0; bit_cnt < sizeof(p_db_lag->member_ports_bits)*8; bit_cnt++)
        {
            if (CTC_BMP_ISSET(p_db_lag->member_ports_bits, bit_cnt))
            {
                status = ctcs_port_set_property(lchip, CTC_MAP_LPORT_TO_GPORT(lchip, bit_cnt), CTC_PORT_PROP_DEFAULT_PCP, (uint32)attr->value.u8);
    ...
    case SAI_LAG_ATTR_DROP_UNTAGGED:
        for (bit_cnt = 0; bit_cnt < sizeof(p_db_lag->member_ports_bits)*8; bit_cnt++)
        {
            if (CTC_BMP_ISSET(p_db_lag->member_ports_bits, bit_cnt))
            {
                CTC_SAI_CTC_ERROR_RETURN( ctcs_port_get_vlan_ctl(lchip, CTC_MAP_LPORT_TO_GPORT(lchip, bit_cnt), (ctc_vlantag_ctl_t*)&val_32));
        ...
        for (bit_cnt = 0; bit_cnt < sizeof(p_db_lag->member_ports_bits)*8; bit_cnt++)
        {
            if (CTC_BMP_ISSET(p_db_lag->member_ports_bits, bit_cnt))
            {
                CTC_SAI_CTC_ERROR_RETURN(ctcs_port_set_vlan_ctl(lchip, CTC_MAP_LPORT_TO_GPORT(lchip, bit_cnt), mode));
            }
        }
...
_ctc_sai_lag_dump_print_cb(ctc_sai_oid_property_t* bucket_data, ctc_sai_db_traverse_param_t *p_cb_data)
{
    ...
    CTC_SAI_LOG_DUMP(p_file, "Vlan_id:%-4d Vlan_pri:%-2d Is_binding_rif:%-2d  Drop_tagged:%-1d  Drop_untagged:%-1d\n", \
    ctc_sai_lag_cur.vlan_id,  ctc_sai_lag_cur.vlan_priority,  ctc_sai_lag_cur.is_binding_rif, ctc_sai_lag_cur.drop_tagged, ctc_sai_lag_cur.drop_untagged);
...
sai_status_t ctc_sai_lag_binding_rif(sai_object_id_t sai_lag_id, uint8 is_binding)
{
    uint8 lchip = 0;
    ctc_sai_lag_info_t*  p_db_lag = NULL;
                                             ...
                                             const sai_attribute_t *attr_list)
{
    ...
    if (SAI_STATUS_SUCCESS == status)
    {
        p_db_lag->vlan_priority = attr->booldata;
    }
    else
    {
        p_db_lag->vlan_priority = false;
    }
...
ctc_sai_lag_notification_all_members_change(uint8 lchip, ctc_sai_lag_mem_change_type_t type, uint32 lag_port, uint32 change)
{
    ...
    if (p_lag->cb[type])
    {
        for (bit_cnt = 0; bit_cnt < sizeof(p_lag->member_ports_bits)*8; bit_cnt++)
        {
            ...
            {
                (p_lag->cb[type])(lchip, lag_port, CTC_MAP_LPORT_TO_GPORT(lchip, bit_cnt), change);
            }
        }
    }
