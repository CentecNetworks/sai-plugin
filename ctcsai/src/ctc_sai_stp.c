...
typedef struct ctc_sai_stp_info_s
{
    ...
    uint32 port_bind_bits[8];
    uint32 vlan_bind_count;
    uint32 port_bind_count;
}ctc_sai_stp_info_t;
...
static sai_status_t ctc_sai_stp_check_port_state(sai_stp_port_state_t state)
{
    ...
    default:
        CTC_SAI_LOG_ERROR(SAI_API_LAG, "Invalid port state passed\n");
...
ctc_sai_stp_get_stp_info(sai_object_key_t *key, sai_attribute_t* attr, uint32 attr_idx)
{
    ...
    case  SAI_STP_ATTR_PORT_LIST:
        ...
        {
            CTC_SAI_LOG_ERROR(SAI_API_STP, "not enough memory , objlist count:%d db entry count:%d\n", attr->value.vlanlist.count ,  p_db_stp->port_bind_count);
            return SAI_STATUS_BUFFER_OVERFLOW;
        }
        bridge_ports =  mem_malloc(MEM_SYSTEM_MODULE, sizeof(sai_object_id_t)*p_db_stp->port_bind_count);
        if (NULL == bridge_ports)
        {
            return SAI_STATUS_NO_MEMORY;
        }
        sal_memset(bridge_ports, 0, sizeof(sai_object_id_t)*p_db_stp->port_bind_count);
        for (bit_cnt = 0; bit_cnt < p_db_stp->port_bind_count; bit_cnt++)
        {
            if (CTC_BMP_ISSET(p_db_stp->port_bind_bits, bit_cnt))
            {
                bridge_ports[port_num] =  ctc_sai_create_object_id(SAI_OBJECT_TYPE_BRIDGE_PORT, ctc_object_id.lchip, 0, 0, bit_cnt);
                port_num++;
            }
        }

        status = ctc_sai_fill_object_list(sizeof(sai_object_id_t), bridge_ports, port_num, &attr->value.objlist);
        mem_free(bridge_ports);
        break;
                                                ...
                                                const sai_attribute_t *attr_list)
{
    ...
    if (SAI_BRIDGE_PORT_TYPE_PORT  != p_bridge_port->port_type /*|| bridge_port->port_type != SAI_BRIDGE_PORT_TYPE_SUB_PORT*/)
    {
    ...
    *stp_port_id = ctc_sai_create_object_id(SAI_OBJECT_TYPE_STP_PORT, lchip, 0, stp_oid.value, gport);
    ...
    if (SAI_STATUS_SUCCESS != status)
    {
        CTC_SAI_LOG_ERROR(SAI_API_STP, "Failed to create stp port entry:%d\n", status);
    }
    return status;
...
static sai_status_t _ctc_sai_stp_remove_stp_port(sai_object_id_t stp_port_id)
{
    ...
    CTC_SAI_DB_LOCK(ctc_stp_port_oid.lchip);
    ...
    CTC_SAI_CTC_ERROR_GOTO(ctcs_stp_set_state(ctc_stp_port_oid.lchip, ctc_stp_port_oid.value, ctc_stp_port_oid.value2, SAI_STP_PORT_STATE_FORWARDING), status, error0);

    error0:
    CTC_BMP_SET(p_db_stp->port_bind_bits, CTC_MAP_GPORT_TO_LPORT(ctc_stp_port_oid.value));
    out:
    CTC_SAI_DB_UNLOCK(ctc_stp_port_oid.lchip);
    if (SAI_STATUS_SUCCESS != status)
    {
...
sai_status_t ctc_sai_stp_set_instance(uint8 lchip, uint16 vlan_ptr, sai_object_id_t stp_oid, uint8 is_add)
{
    ctc_sai_stp_info_t *p_db_stp = NULL;
    ctc_object_id_t ctc_object_id;
    ...
    if (is_add)
    {
        ...
        CTC_BMP_SET(p_db_stp->vlan_bind_bits, vlan_ptr);
        p_db_stp->vlan_bind_count ++;
    }
    else
    {
        CTC_BMP_UNSET(p_db_stp->vlan_bind_bits, vlan_ptr);
        p_db_stp->vlan_bind_count --;
    }
...
out:
    ...
    if (SAI_STATUS_SUCCESS != status)
    {
        CTC_SAI_LOG_ERROR(SAI_API_STP, "Failed to create stp entry:%d\n", status);
    }
    return status;
...
static sai_status_t ctc_sai_stp_remove_stp(sai_object_id_t sai_stp_id)
{
    ...
    p_db_stp = ctc_sai_db_get_object_property(ctc_object_id.lchip, sai_stp_id);
    if (NULL == p_db_stp)
    {
        status = SAI_STATUS_INVALID_OBJECT_ID;
        goto out;
    }
    /*TODO: check for VLANs associated */
    {

    }

    CTC_SAI_ERROR_GOTO(ctc_sai_db_remove_object_property(ctc_object_id.lchip, sai_stp_id ), status, out);
    CTC_SAI_ERROR_GOTO(ctc_sai_db_free_id(ctc_object_id.lchip, CTC_SAI_DB_ID_TYPE_STP, ctc_object_id.value), status, error0);
    mem_free(p_db_stp);
    goto out;
...
out:
    ...
    if (SAI_STATUS_SUCCESS != status)
    {
        CTC_SAI_LOG_ERROR(SAI_API_STP, "Failed to remove stp entry:%d\n", status);
    }
    return status;
}
...
sai_status_t ctc_sai_stp_set_stp_port_attribute(sai_object_id_t sai_stp_id, const sai_attribute_t *attr)
{
    ...
    CTC_SAI_ERROR_RETURN(ctc_sai_oid_get_lchip(sai_stp_id, &lchip));
    CTC_SAI_DB_LOCK(lchip);
    status = ctc_sai_set_attribute(&key,key_str,   SAI_OBJECT_TYPE_STP,  stp_port_attr_fn_entries,attr);
    CTC_SAI_DB_UNLOCK(lchip);
    if (SAI_STATUS_SUCCESS != status)
    {
        CTC_SAI_LOG_ERROR(SAI_API_STP, "Failed to set stp attr:%d, status:%d\n", attr->id,status);
                                           ...
                                           sai_attribute_t   *attr_list)
{
    ...
    {
        CTC_SAI_ERROR_GOTO(ctc_sai_get_attribute(&key, key_str,
                                                 SAI_OBJECT_TYPE_STP, loop, stp_port_attr_fn_entries, &attr_list[loop]), status, out);
        loop++ ;
    }
...
ctc_sai_stp_db_init(uint8 lchip)
{
    ...
    switch_id = ctc_sai_create_object_id(SAI_OBJECT_TYPE_SWITCH, lchip, 0, 0, (uint32) lchip);
