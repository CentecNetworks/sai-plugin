...
typedef struct _ctc_sai_mirr_cb_para_s
{
    ...
    ctc_sai_mirror_session_t*  p_mirr_session;
    ...
    if(CTC_SAI_MIRROR_ALLOC_NH == operate_type)
    {
        ...
        for (loop_i = 0; loop_i < p_mirr_sess_res->session_list_cnt; loop_i++)
        {
            ...
            else if((SAI_MIRROR_SESSION_TYPE_REMOTE == mirr_type) || (SAI_MIRROR_SESSION_TYPE_ENHANCED_REMOTE == mirr_type))
            {
                ...
                nh_mcast_group.mem_info.ref_nhid = p_mirr_sess_res->p_mirr_sess->nh_id;
                CTC_SAI_CTC_ERROR_RETURN(ctcs_nh_update_mcast(lchip, nh_id_mcast, &nh_mcast_group));
            }
...
{
    uint32 nh_id_rspan = 0;
    ctc_rspan_nexthop_param_t ctc_rspan_nexthop_param;

    sal_memset(&ctc_rspan_nexthop_param, 0, sizeof(ctc_rspan_nexthop_param_t));

    if(CTC_SAI_MIRROR_ALLOC_NH == operate_type)
    {
        ctc_rspan_nexthop_param.rspan_vid = p_mirr_session->vlan_id;
        CTC_SAI_ERROR_RETURN(ctc_sai_db_alloc_id(lchip, CTC_SAI_DB_ID_TYPE_NEXTHOP, &nh_id_rspan));
        CTC_SAI_CTC_ERROR_RETURN(ctcs_nh_add_rspan(lchip, nh_id_rspan, &ctc_rspan_nexthop_param));

        p_mirr_sess_res->p_mirr_sess[session_index].nh_id = nh_id_rspan;
    }
    ...
    {
        nh_id_rspan = p_mirr_sess_res->p_mirr_sess[session_index].nh_id;
        CTC_SAI_CTC_ERROR_RETURN(ctcs_nh_remove_rspan(lchip, nh_id_rspan));
        CTC_SAI_ERROR_RETURN(ctc_sai_db_free_id(lchip, CTC_SAI_DB_ID_TYPE_NEXTHOP, nh_id_rspan));
...
_ctc_sai_mirror_lkup_set_attr(ctc_sai_vector_property_t* bucket_data, _ctc_sai_mirr_cb_para_t* cb_para)
{
    ...
    uint8 is_mcast = 0;
    ctc_mcast_nh_param_group_t nh_mcast_group;
    ctc_sai_mirror_session_t* p_mirr_session = cb_para->p_mirr_session;
    ctc_sai_mirr_sess_res_t* mirr_sess_res = bucket_data->data;
    ...
    for (loop_i = 0; loop_i < mirr_sess_res->session_list_cnt; loop_i++)
    {
        ...
        if (SAI_MIRROR_SESSION_TYPE_LOCAL == mirr_type)
        {
            if(is_mcast)
            {
                ...
                CTC_SAI_ERROR_RETURN(ctc_sai_oid_get_gport(p_mirr_session->dst_port, &gport));
                nh_mcast_group.mem_info.destid = gport;
                CTC_SAI_ERROR_RETURN(ctcs_nh_update_mcast(cb_para->lchip, mirr_sess_res->mc_nh_id, &nh_mcast_group));

                /*  second: add new member */
                nh_mcast_group.opcode = CTC_NH_PARAM_MCAST_ADD_MEMBER;
                CTC_SAI_ERROR_RETURN(ctc_sai_oid_get_gport(cb_para->attr->value.oid, &gport));
                nh_mcast_group.mem_info.destid = gport;
                CTC_SAI_ERROR_RETURN(ctcs_nh_update_mcast(cb_para->lchip, mirr_sess_res->mc_nh_id, &nh_mcast_group));
            }
        }
        ...
        else if(SAI_MIRROR_SESSION_TYPE_ENHANCED_REMOTE == mirr_type)
        {
            ...
            CTC_SAI_ERROR_RETURN(_ctc_sai_mirror_operate_nh_iptunnel(cb_para->lchip,
                                                                     CTC_SAI_MIRROR_UPDATE_NH, p_mirr_session, loop_i, mirr_sess_res));
            if(is_mcast)
            {
                nh_mcast_group.mem_info.member_type = CTC_NH_PARAM_MEM_LOCAL_WITH_NH;
                                        ...
                                        ctc_sai_mirror_session_t* p_mir_session)
{
    ...
    if (status == SAI_STATUS_SUCCESS )
    {
        p_mir_session->vlan_cfi = attr_value->u16;
    }
...
_ctc_sai_mirr_set_attr(sai_object_key_t* key, const sai_attribute_t* attr)
{
    ...
    mirr_cb_parm.p_mirr_session = p_mirr_session;
...
_ctc_sai_mirror_binding_mirr_param_chk(uint8 lchip, uint8 binding_module, const sai_attribute_t *attr)
{
    ...
    uint32                   truncated_size_fir = 0xFFFFFFFF;
    uint8 loop_i = 0;
    ...
    if (objlist.count >= 1) /* enable */
    {
        for (loop_i = 0; loop_i < objlist.count; loop_i++)
        {
            ...
            if(0xFFFFFFFF == truncated_size_fir)
            {
                truncated_size_fir = p_mir_session->truncated_size;
            }
            if (truncated_size_fir != p_mir_session->truncated_size)
            {
                CTC_SAI_LOG_ERROR(SAI_API_MIRROR, "Failed to binding mirror session to %s, truncated_size is not same;\
                session_id: 0x%"PRIx64"!\n", objlist.list[loop_i], print_str);
...
_ctc_sai_mirror_session_res_wb_reload_cb(uint8 lchip, void* key, void* data)  /* no need void* key */
{
    ...
    wb_query.buffer = (ctc_wb_key_data_t*)(&mirr_sess_res_wb);
    for (entry_cnt = 0; entry_cnt < p_mirr_sess->session_list_cnt; entry_cnt++)
    {
        wb_query.valid_cnt = 0;
        mirr_sess_res_wb.vec_node_index = p_mirr_sess->vec_node_index;
        mirr_sess_res_wb.index = entry_cnt;
        wb_query.key = (uint8*)(&mirr_sess_res_wb);
        ctc_wb_query_entry(&wb_query);
        if(wb_query.valid_cnt)
        {
            sal_memcpy(&(p_mirr_sess->p_mirr_sess[entry_cnt]), &mirr_sess_res_wb.mirr_sess,  sizeof(ctc_sai_mirr_sess_t));
        }
        else
        {
            mem_free(p_mirr_sess->p_mirr_sess);
...
sai_status_t
//ctc_sai_mirror_set_acl_mirr(uint8 lchip, uint32 acl_entry_id, sai_attribute_t *attr)
ctc_sai_mirror_set_acl_mirr(uint8 lchip, uint8 priority, uint8* ctc_session_id, sai_attribute_t *attr) /* called by acl SAI_ACL_ACTION_TYPE_MIRROR_INGRESS SAI_ACL_ACTION_TYPE_MIRROR_EGRESS */
...
ctc_sai_mirror_db_init(uint8 lchip)
{
    ...
    ctc_sai_warmboot_register_cb(lchip, CTC_SAI_WB_TYPE_VECTOR, SAI_OBJECT_TYPE_MIRROR_SESSION, (void*)(&wb_info));
