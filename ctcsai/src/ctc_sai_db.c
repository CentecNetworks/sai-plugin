...
_ctc_sai_db_entry_mapping_key(ctc_sai_db_entry_type_t type, void* key, ctc_sai_entry_property_t* entry_property)
{
    ...
    else if (CTC_SAI_DB_ENTRY_TYPE_ROUTE == type)
    {
        ...
        ipv6_addr_t mask;
        ctc_sai_get_ctc_object_id(SAI_OBJECT_TYPE_ROUTER_INTERFACE, route_entry->vr_id, &ctc_object_id);
    ...
    else if (CTC_SAI_DB_ENTRY_TYPE_MCAST_IPMC == type)
    {
        ...
        ctc_sai_get_ctc_object_id(SAI_OBJECT_TYPE_ROUTER_INTERFACE, ipmc_entry->vr_id, &ctc_object_id);
...
ctc_sai_db_entry_unmapping_key(uint8 lchip, ctc_sai_db_entry_type_t type, ctc_sai_entry_property_t* entry_property, void* key)
{
    ...
    {
        sai_neighbor_entry_t* neighbor_entry = (sai_neighbor_entry_t*)key;
        neighbor_entry->switch_id = ctc_sai_create_object_id(SAI_OBJECT_TYPE_SWITCH, lchip, 0, 0, lchip);
    ...
    else if (CTC_SAI_DB_ENTRY_TYPE_ROUTE == type)
    {
        sai_route_entry_t* route_entry = (sai_route_entry_t*)key;
        ipv6_addr_t mask;
        route_entry->switch_id = ctc_sai_create_object_id(SAI_OBJECT_TYPE_SWITCH, lchip, 0, 0, lchip);
        route_entry->vr_id = ctc_sai_create_object_id(SAI_OBJECT_TYPE_ROUTER_INTERFACE, lchip, 0, 0, entry_property->key.route.vrf_id);
    ...
    else if (CTC_SAI_DB_ENTRY_TYPE_MCAST_L2MC == type)
    {
        sai_object_type_t oid_type = SAI_OBJECT_TYPE_NULL;
        sai_l2mc_entry_t* l2mc_entry = (sai_l2mc_entry_t*)key;
        l2mc_entry->switch_id = ctc_sai_create_object_id(SAI_OBJECT_TYPE_SWITCH, lchip, 0, 0, lchip);
    ...
    else if (CTC_SAI_DB_ENTRY_TYPE_MCAST_IPMC == type)
    {
        ...
        ipmc_entry->vr_id = ctc_sai_create_object_id(SAI_OBJECT_TYPE_ROUTER_INTERFACE, lchip, 0, 0, entry_property->key.mcast_ip.vrf_id);
...
ctc_sai_db_vector_traverse(uint8 lchip, ctc_sai_db_vector_type_t type, vector_traversal_fn fn, void* data)
{
    ...
    ctc_vector_traverse(g_sai_db[lchip]->vector[type], fn, data);
    return SAI_STATUS_SUCCESS;
}
...
ctc_sai_get_switch_property(uint8 lchip)
{
    ...
    sai_oid = ctc_sai_create_object_id(SAI_OBJECT_TYPE_SWITCH, lchip, 0, 0, 0);
