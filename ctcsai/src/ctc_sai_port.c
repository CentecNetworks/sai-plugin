...
static sai_status_t ctc_sai_port_get_basic_info(  sai_object_key_t   *key, sai_attribute_t *attr, uint32 attr_idx)
{
    ...
    case SAI_PORT_ATTR_SUPPORTED_FEC_MODE:
        ...
        CTC_SAI_ERROR_RETURN(ctc_sai_fill_object_list(sizeof(uint32), fecs, num, &attr->value.u32list));
        break;
    ...
    case SAI_PORT_ATTR_INTERNAL_LOOPBACK_MODE:
        ...
        sal_memset(&serdes_port, 0, sizeof(serdes_port));
        /*TODO:sai_port_internal_loopback_mode_t*/
        CTC_SAI_ATTR_ERROR_RETURN(ctcs_port_get_capability(lchip, gport, CTC_PORT_CAP_TYPE_SERDES_INFO, &serdes_port), attr_idx);
    ...
    case SAI_PORT_ATTR_PRIORITY_FLOW_CONTROL_MODE:
        attr->value.u8 = p_port_db->flow_ctl_mode;
        break;
...
static sai_status_t ctc_sai_port_set_basic_info(  sai_object_key_t   *key, const sai_attribute_t *attr)
{
    ...
    case SAI_PORT_ATTR_INTERNAL_LOOPBACK_MODE:
        ...
        /*TODO:loopback is currently have some problem:only support mac ??*/
        for (num = 0; num < serdes_port.serdes_num; num++)
        {
            lb_param.serdes_id = serdes_port.serdes_id_array[num];
            if (SAI_PORT_INTERNAL_LOOPBACK_MODE_NONE == attr->value.s32)
            {
                lb_param.enable = 0;
            }
            else
    ...
    case SAI_PORT_ATTR_FEC_MODE:
        /*TODO:ctc type is ctc_port_fec_type_e*/
    ...
    case SAI_PORT_ATTR_QOS_SCHEDULER_PROFILE_ID:
        {
            ctc_sai_port_db_t* p_port = NULL;
            CTC_SAI_ERROR_RETURN(_ctc_sai_port_get_port_db(key->key.object_id, &p_port));
            CTC_SAI_ERROR_RETURN(ctc_sai_scheduler_port_set_scheduler(key->key.object_id, attr));
        }
        break;
    case SAI_PORT_ATTR_PRIORITY_FLOW_CONTROL_MODE:
        {
            if(SAI_PORT_PRIORITY_FLOW_CONTROL_MODE_COMBINED == attr->value.u8)
            {
                p_port_db->flow_ctl_mode = SAI_PORT_PRIORITY_FLOW_CONTROL_MODE_COMBINED;
            }
            if(SAI_PORT_PRIORITY_FLOW_CONTROL_MODE_SEPARATE == attr->value.u8)
            {
...
_ctc_sai_port_get_mirror(sai_object_key_t *key, sai_attribute_t *attr, uint32 attr_idx)
{
    ...
    CTC_SAI_ATTR_ERROR_RETURN(ctc_sai_oid_get_gport(key->key.object_id, &gport), attr_idx);
    CTC_SAI_ATTR_ERROR_RETURN(ctc_sai_oid_get_lchip(key->key.object_id, &lchip), attr_idx);

    CTC_SAI_ATTR_ERROR_RETURN(ctc_sai_mirror_get_port_mirr(lchip, gport, attr), attr_idx);
    return SAI_STATUS_SUCCESS;
}
    ...
    switch (attr->id)
    {
        case SAI_PORT_ATTR_QOS_DOT1P_TO_TC_MAP:
            if (enable)
            {
                ...
                if ((chip_type == CTC_CHIP_GOLDENGATE)
                    && (p_port_db->dscp_to_tc_map_id || p_port_db->dscp_to_color_map_id))
                {
        ...
        case SAI_PORT_ATTR_QOS_DSCP_TO_TC_MAP:
            if (enable)
            {
                ...
                if ((chip_type == CTC_CHIP_GOLDENGATE)
                    && (p_port_db->dot1p_to_tc_map_id || p_port_db->dot1p_to_color_map_id))
                {
                    //GG not support revert qos domain
        ...
        case SAI_PORT_ATTR_QOS_TC_AND_COLOR_TO_DOT1P_MAP:
            if (enable)
            {
                ...
                if ((chip_type == CTC_CHIP_GOLDENGATE)
                    && p_port_db->tc_color_to_dscp_map_id)
                {
                    //GG not support revert qos domain
        ...
        case SAI_PORT_ATTR_QOS_TC_AND_COLOR_TO_DSCP_MAP:
            if (enable)
            {
                ...
                if ((chip_type == CTC_CHIP_GOLDENGATE)
                    && p_port_db->tc_color_to_dot1p_map_id)
                {
                    //GG not support revert qos domain
...
_ctc_sai_port_get_global_flow_ctl(sai_object_key_t *key, sai_attribute_t *attr, uint32 attr_idx)
{
    ...
    if (igs_en && egs_en)
    {
        attr->value.u32 = SAI_PORT_FLOW_CONTROL_MODE_BOTH_ENABLE;
    }
    else if (igs_en)
    {
        attr->value.u32 = SAI_PORT_FLOW_CONTROL_MODE_RX_ONLY;
    }
    else if (egs_en)
    {
        attr->value.u32 = SAI_PORT_FLOW_CONTROL_MODE_TX_ONLY;
    }
    else
    {
        attr->value.u32 = SAI_PORT_FLOW_CONTROL_MODE_DISABLE;
    }
    return SAI_STATUS_SUCCESS;
}
...
static  ctc_sai_attr_fn_entry_t  port_attr_fn_entries[] =
{
    ...
    {SAI_PORT_ATTR_REMOTE_ADVERTISED_SPEED,                        ctc_sai_port_get_basic_info,  NULL},
    ...
    {SAI_PORT_ATTR_QOS_DEFAULT_TC,                                 _ctc_sai_port_get_qos_map,  _ctc_sai_port_set_qos_map},
    ...
    {SAI_PORT_ATTR_QOS_TC_TO_QUEUE_MAP,                            _ctc_sai_port_get_qos_map,  _ctc_sai_port_set_qos_map},
    ...
    {SAI_PORT_ATTR_QOS_TC_TO_PRIORITY_GROUP_MAP,                   _ctc_sai_port_get_qos_map,  _ctc_sai_port_set_qos_map},
    {SAI_PORT_ATTR_QOS_PFC_PRIORITY_TO_PRIORITY_GROUP_MAP,         _ctc_sai_port_get_qos_map,  _ctc_sai_port_set_qos_map},
    {SAI_PORT_ATTR_QOS_PFC_PRIORITY_TO_QUEUE_MAP,                  _ctc_sai_port_get_qos_map,  _ctc_sai_port_set_qos_map},
                         ...
                         const sai_attribute_t *attr_list)
{
    ...
    if (NULL == port_id)
    {
        CTC_SAI_LOG_ERROR(SAI_API_PORT, "NULL port id param\n");
    ...
    for (num = 0; num < local_panel_ports.count; num++)
    {
        ...
        {
            *port_id = ctc_sai_create_object_id(SAI_OBJECT_TYPE_PORT, lchip, 0, 0, gport);
            break;
        }
    }
...
ctc_sai_port_db_init(uint8 lchip)
{
    ...
    for(port_idx = 0; port_idx < local_panel_ports.count; port_idx++)
    {
        ...
        //should not ERROR_RETURN if WB reload
