...
ctc_sai_switch_fdb_aging_process(uint8 gchip, void* p_data)
{
    ...
    for (i = 0; i < p_fifo->fifo_idx_num; i++)
    {
        ...
        if (p_entry->aging_type == CTC_AGING_TYPE_MAC)
        {
            ...
            if (NULL == data)
            {
                fdb_events[index].fdb_entry.bv_id = ctc_sai_create_object_id(SAI_OBJECT_TYPE_BRIDGE, lchip, 0, 0, p_entry->fid);
            }
            fdb_events[index].fdb_entry.switch_id = ctc_sai_create_object_id(SAI_OBJECT_TYPE_SWITCH, lchip, 0, 0, lchip);
...
ctc_sai_switch_fdb_learning_process(uint8 gchip, void* p_data)
{
    ...
    for (index = 0; index < p_cache->entry_num; index++)
    {
        ...
        if (p_cache->learning_entry[index].is_logic_port)
        {
            ...
            fdb_events[index].fdb_entry.bv_id = ctc_sai_create_object_id(SAI_OBJECT_TYPE_BRIDGE, lchip, 0, 0, l2_addr.fid);
            if (!CTC_FLAG_ISSET(p_switch_master->flag, CTC_SAI_SWITCH_FLAG_HW_LEARNING_EN))
            {
        ...
        fdb_events[index].fdb_entry.switch_id = ctc_sai_create_object_id(SAI_OBJECT_TYPE_SWITCH, lchip, 0, 0, 0);
...
ctc_sai_switch_get_global_panel_ports(sai_object_key_t* key, sai_attribute_t* attr, uint32 attr_idx)
{
    ...
    switch(attr->id)
    {
        ...
        case SAI_SWITCH_ATTR_PORT_LIST:
            ...
            {
                ports[num] = ctc_sai_create_object_id(SAI_OBJECT_TYPE_PORT, lchip, 0, 0, CTC_MAP_LPORT_TO_GPORT(gchip, local_panel_ports.lport[num]));
            }
...
ctc_sai_switch_get_global_property(sai_object_key_t* key, sai_attribute_t* attr, uint32 attr_idx)
{
    ...
    switch(attr->id)
    {
        ...
        case SAI_SWITCH_ATTR_FDB_UNICAST_MISS_PACKET_ACTION:
            attr->value.u32 = p_switch_master->fdb_miss_action[0];
            break;
        case SAI_SWITCH_ATTR_FDB_BROADCAST_MISS_PACKET_ACTION:
            ...
            attr->value.u32 = p_switch_master->fdb_miss_action[2];
            break;
        case SAI_SWITCH_ATTR_FDB_MULTICAST_MISS_PACKET_ACTION:
            attr->value.u32 = p_switch_master->fdb_miss_action[1];
            break;
        /*meta data range*/
        case SAI_SWITCH_ATTR_FDB_DST_USER_META_DATA_RANGE:
            attr->value.u32 = CTC_SAI_META_DATA_FDB_DST_MAX;
            break;
        case SAI_SWITCH_ATTR_ROUTE_DST_USER_META_DATA_RANGE:
            attr->value.u32 = CTC_SAI_META_DATA_ROUTE_DST_MAX;
            break;
        case SAI_SWITCH_ATTR_NEIGHBOR_DST_USER_META_DATA_RANGE:
            attr->value.u32 = CTC_SAI_META_DATA_NEIGHBOR_DST_MAX;
            break;
        case SAI_SWITCH_ATTR_PORT_USER_META_DATA_RANGE:
            attr->value.u32 = CTC_SAI_META_DATA_PORT_MAX;
            break;
        case SAI_SWITCH_ATTR_VLAN_USER_META_DATA_RANGE:
            attr->value.u32 = CTC_SAI_META_DATA_VLAN_MAX;
            break;
        case SAI_SWITCH_ATTR_ACL_USER_META_DATA_RANGE:
            attr->value.u32 = CTC_SAI_META_DATA_ACL_MAX;
            break;
        ...
        case SAI_SWITCH_ATTR_SWITCH_HARDWARE_INFO:
            ...
            else
            {
                ...
                CTC_SAI_ERROR_RETURN(ctc_sai_fill_object_list(sizeof(uint8), pci_no, 4, &attr->value.u8list));
            }
            break;
...
ctc_sai_switch_get_hash_property(sai_object_key_t* key, sai_attribute_t* attr, uint32 attr_idx)
{
    ...
    switch(attr->id)
    {
        ...
        case SAI_SWITCH_ATTR_ECMP_HASH: /* sub type: 0 */
            attr->value.oid = ctc_sai_create_object_id(SAI_OBJECT_TYPE_HASH, lchip, SAI_SWITCH_ATTR_ECMP_HASH,0,0);
            break;
        case SAI_SWITCH_ATTR_LAG_HASH:  /* sub type: 1 */
            attr->value.oid = ctc_sai_create_object_id(SAI_OBJECT_TYPE_HASH, lchip, SAI_SWITCH_ATTR_LAG_HASH,0,0);
            break;
        /*ecmp*/
...
ctc_sai_switch_get_callback_event(sai_object_key_t* key, sai_attribute_t* attr, uint32 attr_idx)
{
    ...
    switch(attr->id)
    {
        ...
        case SAI_SWITCH_ATTR_TAM_EVENT_NOTIFY:
            attr->value.ptr = p_switch_master->tam_event_cb;
            break;
        default:
            CTC_SAI_LOG_ERROR(SAI_API_SWITCH, "switch attribute %d not implemented\n", attr->id);
            status = SAI_STATUS_ATTR_NOT_IMPLEMENTED_0+attr_idx;
...
ctc_sai_switch_init_switch(uint8 lchip, dal_pci_dev_t* p_pci_dev, sai_object_id_t switch_id)
{
    ...
    CTC_SAI_ERROR_GOTO(ctc_sai_swith_isr_init(lchip), status, roll_back_1);
    if (!reloading)
    {
    ...
    CTC_SAI_CTC_ERROR_GOTO(ctc_sai_warmboot_init(lchip, reloading), status, roll_back_1);/*because ctcs_sdk_init call ctc_wb_init_done*/
    CTC_SAI_CTC_ERROR_GOTO(ctc_sai_switch_create_db(lchip), status, roll_back_1);
    CTC_SAI_CTC_ERROR_GOTO(ctc_sai_warmboot_init_done(lchip, reloading), status, roll_back_2);
                                        ...
                                        const sai_attribute_t* attr_list)
{
    ...
    CTC_SAI_ERROR_RETURN(ctc_sai_find_attrib_in_list(attr_count, attr_list, SAI_SWITCH_ATTR_INIT_SWITCH, &attr_val, &attr_idx));
    ...
    {
        index = attr_val->u8list.count;
        if ((1 == index) || (4 == index))
        {
            ctc_sai_switch_id.lchip = attr_val->u8list.list[0];
            ctc_sai_switch_id.value = attr_val->u8list.list[0];
            if (index > 1)
            {
                pci_dev.busNo = attr_val->u8list.list[1];
                pci_dev.devNo = attr_val->u8list.list[2];
                pci_dev.funNo = attr_val->u8list.list[3];
            }
        }
    }
    else
    ...
    while (loop_i < (SAI_SWITCH_ATTR_FDB_MULTICAST_MISS_PACKET_ACTION - SAI_SWITCH_ATTR_FDB_UNICAST_MISS_PACKET_ACTION + 1))
    {
 ...
 */
 ...
 ctc_sai_switch_get_switch_attribute(sai_object_id_t switch_id, sai_uint32_t attr_count, sai_attribute_t* attr_list)
 {
    ...
    {
        ctc_sai_get_attribute(&key, NULL,
              SAI_OBJECT_TYPE_SWITCH, loop, switch_attr_fn_entries, &attr_list[loop]);
        loop++;
    }
    CTC_SAI_DB_UNLOCK(lchip);
...
void ctc_sai_switch_dump(uint8 lchip, sal_file_t p_file, ctc_sai_dump_grep_param_t *dump_grep_param)
{
    ...
    if (CTC_BMP_ISSET(dump_grep_param->object_bmp, SAI_OBJECT_TYPE_SWITCH))
    {
        ...
        sal_memset(&local_panel_ports, 0, sizeof(local_panel_ports));
        switch_oid_cur = ctc_sai_create_object_id(SAI_OBJECT_TYPE_SWITCH, lchip, 0, 0, lchip);
        ...
        for(loop_count = 0; loop_count<local_panel_ports.count/2; loop_count++)
        {
            CTC_SAI_LOG_DUMP(p_file, "%-14d%-7s| %-14d%-9s\n", loop_count,
                ctc_switch_master_cur.lport_link_status[loop_count]? "up":"down",
                loop_count+((local_panel_ports.count/2)),
                ctc_switch_master_cur.lport_link_status[loop_count+((local_panel_ports.count/2))]? "up":"down");
        }
